...
JOIN ICH_AKI_label a ON f.subject_id = a.subject_id AND f.hadm_id = a.hadm_id 
JOIN LabEvents l ON f.subject_id = l.subject_id AND f.hadm_id = l.hadm_id
...
FROM ICH_First_ICU_Stay f 
JOIN ICH_AKI_label a ON f.subject_id = a.subject_id AND f.hadm_id = a.hadm_id 
JOIN ChartEvents c ON f.subject_id = c.subject_id AND f.hadm_id = c.hadm_id AND f.stay_id = c.stay_id
